...
function zerif_customize_register( $wp_customize ) {
	...
	if ( class_exists( 'WP_Customize_Panel' ) ):
		...
		$wp_customize->add_setting( 'zerif_bigtitle_minititle', array(
			'sanitize_callback' => 'zerif_sanitize_input',
			'default' => __('ONE OF THE TOP 10 MOST POPULAR THEMES ON WORDPRESS.ORG','zerif-lite'),
			'transport' => 'postMessage'
		));

		$wp_customize->add_control( 'zerif_bigtitle_minititle', array(
			'label'    => __( 'Mini Title', 'zerif-lite' ),
			'section'  => 'zerif_bigtitle_section',
			'priority'    => 3,
		));
	...
	if ( class_exists( 'WP_Customize_Panel' ) ):
		...
		$wp_customize->add_panel( 'panel_ourfocus', array(
			'priority' => 32,
			'capability' => 'edit_theme_options',
			'title' => __( 'Feature of Linkcare section', 'zerif-lite' )
		));
	...
	if ( class_exists( 'WP_Customize_Panel' ) ):
		...
		$wp_customize->add_panel( 'panel_ourteam', array(
			'priority' => 35,
			'capability' => 'edit_theme_options',
			'title' => __( 'User and Partner Section', 'zerif-lite' )
		));
